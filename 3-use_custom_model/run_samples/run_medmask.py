...
def frame_process(frame, tensor):
    ...
    for i in range(100): # There is 100 detections, not all of them are relevant
        if (tensor[i*7 + 2] >0.6): # 3rd value of each detection is the confidence
            keeped_roi.append(tensor[i*7:i*7+7])

    for  id, label, confidence, left, top, right, bottom  in  keeped_roi:
        top_left = (int(left*frame_width), int(top*frame_height))
        bottom_right = (int(right*frame_width), int(bottom*frame_height))
        frame = cv2.rectangle(frame, top_left, bottom_right, (255,0,0), 2) # ROI
        frame = cv2.putText(frame, labels[int(label)], top_left, cv2.FONT_HERSHEY_TRIPLEX, 1.5, (200,0,160)) # Label
    return frame



# Define program parameters
nn_path = str(Path(__file__).parent) + "/../models/medmask/medmask.blob" # path to the neural network compiled model (.blob)
...
nn_output_stream.setStreamName("detection")
...
with dai.Device(pipeline) as device:
    ...
    detection_queue = device.getOutputQueue(name="detection", maxSize=4, blocking=False)
