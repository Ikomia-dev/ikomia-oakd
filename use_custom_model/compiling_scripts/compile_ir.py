...
if(len(sys.argv) < 4):
    print("this script need 3 parameters : model.xml model.bin compiled_model.blob")
else:
    ...
    if(not xml_path.endswith(".xml")):
        print("1st parameter must be a xml file (from OpenVino IR)")
        exit()
    if(not bin_path.endswith(".bin")):
        print("2nd parameter must be a bin file (from OpenVino IR)")
        exit()
