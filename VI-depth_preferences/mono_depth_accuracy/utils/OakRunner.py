...
class OakRunner:
    ...
    def setMonoDepth(self, stream_name="depth", sensor_resolution=dai.MonoCameraProperties.SensorResolution.THE_720_P, input_queue_size=1, block_input_queue=False, output_queue_size=1, block_output_queue=False):
        # Warn user
        if((self.__left_cam is not None and self.__right_cam is None and self.__left_cam.getResolution() != sensor_resolution)
         ...
         or (self.__left_cam is not None and self.__right_cam is not None and self.__right_cam.getResolution() != self.__right_cam.getResolution())):
            ...
            # Set output stream
            stereo_output_stream = self.__pipeline.createXLinkOut()
            stereo_output_stream.setStreamName(stream_name)
            self.__stereo.disparity.link(stereo_output_stream.input)
            self.__stereo_stream_name = stream_name
            self.__prepareStreamQueue(stream_name, output_queue_size, block_output_queue, self.__output_queue_arguments)
    ...
    def run(self, process, init=None):
        ...
        with dai.Device(self.__pipeline) as device:
            ...
            if(self.__stereo_stream_name is not None):
                self.output_queues[self.__stereo_stream_name] = device.getOutputQueue(name=self.__stereo_stream_name, maxSize=self.__output_queue_arguments[self.__stereo_stream_name]["maxSize"], blocking=self.__output_queue_arguments[self.__stereo_stream_name]["blocking"])
